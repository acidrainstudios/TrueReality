...
#ifndef PHYSICS_GENERIC_ACTOR_CPP
#define PHYSICS_GENERIC_ACTOR_CPP

#include "../Include/PhysicsActorGeneric.h"


///////////////////////////////////////////////////////////////////////////////
const std::string PhysicsGenericActor::ACTOR_DEFAULT_NAME("Generic Actor");
const std::string PhysicsGenericActor::ACTOR_CLASS_NAME("vrsPacifPhysics::PhysicsGenericActor");
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
PhysicsGenericActor::PhysicsGenericActor(vrsCore::GameStaticMeshActorProxy& proxy)
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::OnTickLocal(const dtGame::TickMessage& tickMessage)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::ApplyDampForces()
{
	float dampening = 0.6;
	float lesserDampening = 0.995;
	float boatDampening = 0.9;
	...
	if (bDriving)
	{
		if (mBoatBeingDriven->GetLinearVelocity().length() > 30)
		{
			osg::Vec3 boatDamp = mBoatBeingDriven->GetLinearVelocity() * boatDampening;
			boatDamp = osg::Vec3(boatDamp.x(), boatDamp.y(), mBoatBeingDriven->GetLinearVelocity().z());
			mBoatBeingDriven->SetLinearVelocity(boatDamp);
		}
	}

}
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::OnTickRemote(const dtGame::TickMessage& tickMessage)
{
   mDeltaSimTime = tickMessage.GetDeltaSimTime();
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::ProcessMessage(const dtGame::Message& message)
{
	//std::cout << "ag here5" << std::endl;
		...
		if (eventMsg.GetGameEvent() != NULL)
		{
			...
			if (eventMsg.GetGameEvent()->GetName() == vrsCore::GAME_EVENT_IGNITION)
			{
				...
				if (!bDriving)
				{
					...
					ray.SetCollisionGroupFilter(7);

					dtPhysics::PhysicsWorld::GetInstance().TraceRay(ray, report);

					std::cout << "dist: " << report.mDistance << std::endl;
					std::cout << "hit: " << report.mHasHitObject << std::endl;
					std::cout << "where: " << report.mHitPos << std::endl;
					if (report.mHitObject != 0)
					{
						std::cout << "what: " << report.mHitObject->GetName() << std::endl;
						if (report.mHitObject->GetName().compare("BoatPhysObj") == 0)
						{
							mBoatBeingDriven = &(*(report.mHitObject));
							bDriving = true;
							dtCore::Transform xForm;
							mBoatBeingDriven->GetTransform(xForm);
							SetTransform(xForm);
							GetGameActorProxy().SetRotation(osg::Vec3(0,0,-90));
						}
					}
				}
				else
				{
					GetGameActorProxy().SetTranslation(GetGameActorProxy().GetTranslation() + osg::Vec3(0,0,20));
					dtCore::Transform xform;
					GetTransform(xform);
					mPhysObj->SetTransform(xform);
					mPhysObj->SetLinearVelocity(mBoatBeingDriven->GetLinearVelocity());
			...
			if(eventMsg.GetGameEvent()->GetName() == vrsCore::GAME_EVENT_ROLLRIGHT)
			{
				...
				{
					//mRollRateDirection = -1.0f;
					mBoatBeingDriven->AddTorque(osg::Vec3(0,0,500));
					//mPhysObj->AddTorque(osg::Vec3(0,0,500));

					//mBoatBeingDriven->ApplyImpulse(mBoatBeingDriven->GetLinearVelocity() * -0.1f);
				}
			}
...
void PhysicsGenericActor::SetUpPhysics()
{
	...
	mPhysObj->SetPrimitiveType(dtPhysics::PrimitiveType::SPHERE);
	mPhysObj->SetExtents(osg::Vec3(10, 10, 10));
	mPhysObj->SetOriginOffset(osg::Vec3(0, 0, 5));
	mPhysObj->SetMechanicsType(dtPhysics::MechanicsType::DYNAMIC);
	mPhysObj->SetCollisionGroup(1);
	mPhysObj->SetNotifyCollisions(true);
	mPhysObj->SetName("PlayerObj");
	mPhysObj->CreateFromProperties(GetOSGNode());
	mPhysObj->SetActive(true);

	physicsComponent->RegisterHelper(*mPhysHelper);

	mPhysHelper->SetPrePhysicsCallback(dtUtil::MakeFunctor(&PhysicsGenericActor::BeforePhys, this));
	mPhysHelper->SetPostPhysicsCallback(dtUtil::MakeFunctor(&PhysicsGenericActor::AfterPhys, this));

	//mSplash = dtCore::ParticleSystem::GetInstance("Splash");
	////std::cout << "splash: " << mSplash << std::endl;
	//if (mSplash)
	//{
...
void PhysicsGenericActor::BeforePhys()
{
	if (!bDriving)
	{
		mPhysObj->SetTranslation(GetGameActorProxy().GetTranslation());
	}
	else
	{
		//dtCore::Transform xform;

		//float yaw = GetGameActorProxy().GetRotation().z() + 90;
		//mBoatBeingDriven->SetRotation(osg::Vec3(yaw, 0, 0));

		/*dtCore::Transform xform;
		GetTransform(xform);
		mPhysObj->SetTransform(xform);*/

		//GetGameActorProxy().SetRotation(mPhysObj->GetRotation());
	}
}

void PhysicsGenericActor::AfterPhys()
{
	if (!bDriving)
	{
		GetGameActorProxy().SetTranslation(mPhysObj->GetTranslation());
	}
	else
	{
		GetGameActorProxy().SetTranslation(mBoatBeingDriven->GetTranslation() + osg::Vec3(0, 0, 5));
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::RayHitCall(bool b, dtPhysics::RayCast::Report rep)
{
}
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::MoveForward()
{
	if (mPhysicsHasStarted)
	{
		dtCore::Transform xform;
		...
		if (bDriving && mBoatBeingDriven != 0)
		{
			float x = cos(mBoatBeingDriven->GetRotation().x() * 0.0174532924);
			float y = sin(mBoatBeingDriven->GetRotation().x() * 0.0174532924);
			
			mBoatBeingDriven->ApplyImpulse(osg::Vec3(x,y,0) * 250);
		}
	}
}
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::MoveBackward()
{
	if (mPhysicsHasStarted)
	{
		dtCore::Transform xform;
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::AntiGravity()
{
	if (mPhysicsHasStarted)
	{
		if (bAntiGravity)
		{
			dtPhysics::VectorType antiGrav = *(new dtPhysics::VectorType(0,0,10));
			mPhysObj->AddForce(antiGrav * mPhysObj->GetMass());
			//mPhysObj->SetGravityEnabled(false);	// ...sassy
		}
	}
}
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::ComputeVelocityAndTurn(float deltaSimTime)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::MoveTheActor(float deltaSimTime)
{
	osg::Vec3 pos;
	osg::Matrix rot;
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetVelocity(float velocity)
{
   mVelocity = velocity;
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetVelocityMax(float velocity)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetStrafeRLVelocity(float velocity)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetStrafeUDVelocity(float velocity)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetStrafeVelocityMax(float velocity)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetTurnDirection(int direction)
{
	mTurnRateDirection = direction;
}
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetTurnRate(float rate)
{
	if (mTurnRate != rate)
   {
	  mTurnRate = rate;
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetTurnRateMax(float rate)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetPitchRate(float rate)
{
	if (mPitchRate != rate)
   {
	  mPitchRate = rate;
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetPitchRateMax(float rate)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetRollRate(float rate)
{
	if (mRollRate != rate)
   {
	  mRollRate = rate;
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetRollRateMax(float rate)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetAutoLevelingRate(float rate)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetActorIsPlayer(bool Player)
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::SetCameraOffset(osg::Vec3 offset)
{
	mCameraOffset = offset;
}
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::OnEnteredWorld()
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::FireSetPlayerMessage()
{
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActor::FireSetFocusMessage()
{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
PhysicsGenericActorProxy::PhysicsGenericActorProxy()
{
	SetClassName(PhysicsGenericActor::ACTOR_CLASS_NAME); //Set the actors Class Name
}

///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActorProxy::BuildPropertyMap()
{
	...
	PhysicsGenericActor* actor = static_cast<PhysicsGenericActor*>(GetActor());
   
	// "Is Player" property
	AddProperty(new dtDAL::BooleanActorProperty("IsPlayer","Is Player",
		dtDAL::BooleanActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetActorIsPlayer),
		dtDAL::BooleanActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetActorIsPlayer),
	...
	AddProperty(new dtDAL::BooleanActorProperty("SkyBoxSpin","Sky Box Spin",
		dtDAL::BooleanActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetSkyBoxSpin),
		dtDAL::BooleanActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetSkyBoxSpin),
	...
	AddProperty(new dtDAL::Vec3ActorProperty("CameraOffset","Camera Offset",
		dtDAL::Vec3ActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetCameraOffset),
		dtDAL::Vec3ActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetCameraOffset),
	...
	AddProperty(new dtDAL::FloatActorProperty("MaxVelocity","Maximum Velocity (m/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetVelocityMax),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetVelocityMax),
	...
	AddProperty(new dtDAL::FloatActorProperty("CurrentVelocity","Current Velocity (m/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetVelocity),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetVelocity),
	...
	AddProperty(new dtDAL::FloatActorProperty("MaxStrafeVelocity","Maximum Strafing Velocity (m/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetStrafeVelocityMax),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetStrafeVelocityMax),
	...
	AddProperty(new dtDAL::FloatActorProperty("CurrentLRStrafeVelocity","Current Left/Right Strafing Velocity (m/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetStrafeRLVelocity),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetStrafeRLVelocity),
	...
	AddProperty(new dtDAL::FloatActorProperty("CurrentUDStrafeVelocity","Current Up/Down Strafing Velocity (m/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetStrafeUDVelocity),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetStrafeUDVelocity),
	...
	AddProperty(new dtDAL::FloatActorProperty("MaxTurnRate","Maximum Turn Rate (deg/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetTurnRateMax),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetTurnRateMax),
	...
	AddProperty(new dtDAL::FloatActorProperty("CurrentTurnRate","Current Turn Rate (deg/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetTurnRate),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetTurnRate),
	...
	AddProperty(new dtDAL::FloatActorProperty("MaxPitchRate","Maximum Pitch Rate (deg/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetPitchRateMax),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetPitchRateMax),
	...
	AddProperty(new dtDAL::FloatActorProperty("CurrentPitchRate","Current Pitch Rate (deg/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetPitchRate),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetPitchRate),
	...
	AddProperty(new dtDAL::FloatActorProperty("MaxRollRate","Maximum Roll Rate (deg/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetRollRateMax),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetRollRateMax),
	...
	AddProperty(new dtDAL::FloatActorProperty("CurrentRollRate","Current Roll Rate (deg/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetRollRate),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetRollRate),
	...
	AddProperty(new dtDAL::FloatActorProperty("AutoLevelingRate","Auto Leveling Rate (deg/sec)",
		dtDAL::FloatActorProperty::SetFuncType(actor, &PhysicsGenericActor::SetAutoLevelingRate),
		dtDAL::FloatActorProperty::GetFuncType(actor, &PhysicsGenericActor::GetAutoLevelingRate),
...
///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActorProxy::CreateActor()
{
   SetActor(*new PhysicsGenericActor(*this));
}

///////////////////////////////////////////////////////////////////////////////
void PhysicsGenericActorProxy::OnEnteredWorld()
{
...
#endif //PHYSICS_GENERIC_ACTOR_CPP