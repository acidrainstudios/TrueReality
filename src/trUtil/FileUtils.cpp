...
namespace trUtil
{
        ...
        if (!IsSameFile(strSrc, strDest))
        {
            ...
            try
            {
                ...
                if (ft == DIRECTORY)
                {
                    if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
                    {
                        mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__, "Destination is a directory.");
                    }
                ...
                else
                {
                    if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
                    {
                        mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__, "Destination is a file.");
                    }
                }
                ...
                try
                {
                    if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
                    {
                        mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, "Destination opened for reading.");
                    }
    ...
    void FileUtils::FileMove(const std::string& strSrc, const std::string& strDest, bool bOverwrite) const
    {
        ...
        if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
        {
            mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, "Rename failed, attempting to copy file and delete the source");
        }
    ...
    std::string FileUtils::ArchiveRelativeToAbsolute(const std::string& relativeFile) const
    {
        ...
        trUtil::StringUtils::StringTokenizer<IsFileSeparator>::tokenize(tokens, relativeFile);
    ...
    const struct FileInfo FileUtils::GetFileInfo_Internal(const std::string& strFile, bool caseInsensitive) const
    {
        ...
        else
        {
            ...
            {
                // throw trUtil::FileNotFoundException( std::string("Cannot open file ") + choppedStr);
    ...
    void FileUtils::MakeDirectoryEX(std::string strDir)
    {
        ...
        // prepare tokenizer
        auto buffer = new char[strDir.length() + 1];
    ...
    void FileUtils::ChangeDirectoryInternal(const std::string& path)
    {
        ...
        if (inArchive || !archiveName.empty())
        {
            ...
            if (a != nullptr)
            {
                if (inArchive)
                {
                    ...
                    {
                        throw trUtil::FileNotFoundException(std::string("Cannot change directory into path: ") + path, __FILE__, __LINE__);
                    }
                }
                else
                {
            ...
            {
                throw trUtil::FileNotFoundException(std::string("Cannot find valid archive for path: ") + path, __FILE__, __LINE__);
            }
        }
        else
        {
            ...
            {
                throw trUtil::FileNotFoundException(std::string("Cannot open directory ") + path, __FILE__, __LINE__);
            }
            char buf[512];
            char* bufAddress = getcwd(buf, 512);
            if (buf != bufAddress)
            {
                throw trUtil::FileUtilIOException(std::string("Cannot get current working directory"), __FILE__, __LINE__);
            }
        ...
        if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
        {
            ...
            message += "\".";
            mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__, message.c_str());
        }
    }
    ...
    void FileUtils::PopDirectory()
    {
        if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
        {
            mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__, "Popping Directory.");
        }
    ...
    std::string FileUtils::GetAbsolutePath(const std::string& relativePath, bool removeFinalFile) const
    {
        ...
        {
            throw trUtil::FileNotFoundException(std::string("Cannot open file or directory ") + relativePath, __FILE__, __LINE__);
        }
        else if (fi.fileType == ARCHIVE || fi.isInArchive)
        {
            throw trUtil::FileUtilIOException(std::string("Unable to get the absolute path to a file in an archive. ") + relativePath, __FILE__, __LINE__);
        }
        ...
        {
            throw trUtil::FileUtilIOException(std::string("Cannot get absolute path for file.  Cannot enter directory: ") + relativePath, __FILE__, __LINE__);
        }
        return result;
    }
        ...
        const FileExtensionList& extensions) const
    {
        ...
        while (dirItr != dirContents.end())
        {
            ...
            while (extItr != extensions.end())
            {
                ...
                if (trUtil::StringUtils::StrCompare(testExt, validExt, false) == 0)
                {
        ...
        const std::string& destPath, bool bOverwrite) const
    {
        ...
        for (DirectoryContents::iterator i = contents.begin(); i != contents.end(); ++i)
        {
            ...
            if (fi.fileType == DIRECTORY)
            {
                ...
                {
                    if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_WARNING))
                    {
                        mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_WARNING, __FUNCTION__, __LINE__, "Can't copy %s into itself.", newSrcPath.c_str());
                    }
                }
                else
                {
        ...
        const std::string& destPath, bool bOverwrite, bool copyContentsOnly) const
    {
        ...
        if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
        {
            mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__, "Source directory \"%s\" exists.", fullSrcPath.c_str());
        }
        ...
        if (createDest)
        {
            ...
            if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
            {
                mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__, "Destination directory \"%s\" has been created.",
                    fullDestPath.c_str());
            }
        }
        else
        {
            ...
            if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
            {
                mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__, "Destination directory \"%s\" exists.",
                    fullDestPath.c_str());
            }
            ...
            if (!copyContentsOnly)
            {
                ...
                if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
                {
                    mLogger->LogMessage(trUtil::Logging::LogLevel::LOG_DEBUG, __FUNCTION__, __LINE__,
    ...
    bool FileUtils::DirDelete(const std::string& strDir, bool bRecursive)
    {
        if (bRecursive)
        {
            ...
            catch (const trUtil::FileNotFoundException& ex)
            {
                ...
                if (!DirExists(strDir))
                {
                    if (mLogger->IsLevelEnabled(trUtil::Logging::LogLevel::LOG_DEBUG))
                    {
                        ...
                        ex.LogException(trUtil::Logging::LogLevel::LOG_DEBUG);
                    }
                    return true;
                }
            }
    ...
    void FileUtils::MakeDirectory(const std::string& strDir) const
    {
        if (!iMakeDirectory(strDir))
        {
            ...
            if (ft == REGULAR_FILE)
            {
                throw trUtil::FileUtilIOException(std::string("Cannot create directory. ")
            ...
            {
                throw trUtil::FileNotFoundException(std::string("Cannot create directory ")
                    + strDir + ". Parent directory doesn't exist.", __FILE__, __LINE__);
            }
            else
            {
                throw trUtil::FileUtilIOException(std::string("Cannot create directory ") + strDir + ".", __FILE__, __LINE__);
            }
        }
    }
    ...
    std::string FileUtils::RelativePath(const std::string& absolutePath, const std::string& file) const
    {
        ...
        trUtil::StringUtils::StringTokenizer<IsPathSeparator>::tokenize(absPathVec, absolutePath, pathSepFunc);
        trUtil::StringUtils::StringTokenizer<IsPathSeparator>::tokenize(filePathVec, file, pathSepFunc);
    ...
    FileUtils::FileUtils()
    {
        mLogger = &trUtil::Logging::Log::GetInstance(std::string("fileutils.cpp"));
    ...
    ////////////////////////////////////////////////////////////////////////////////
    trUtil::FileType FileUtils::GetFileTypeForFileInArchive(const osgDB::ArchiveExtended& a, const std::string& path) const
    {
    ...
    ////////////////////////////////////////////////////////////////////////////////
    trUtil::FileInfo FileUtils::GetFileInfoForFileInArchive(const osgDB::ArchiveExtended& a, const std::string& strFile) const
    {
        trUtil::FileInfo info;
    ...
    {
        osg::ref_ptr<osgDB::Archive> archiveResult = nullptr;
    ...
    ////////////////////////////////////////////////////////////////////////////////
    trUtil::FileType FileUtils::GetFileTypeFromOSGDBFileType(osgDB::FileType ft) const
    {
        if (ft == osgDB::REGULAR_FILE)
        {
            return trUtil::REGULAR_FILE;
        }
        else if (ft == osgDB::DIRECTORY)
        {
            return trUtil::DIRECTORY;
        }
        else
        {
            return trUtil::FILE_NOT_FOUND;
        }
            ...
            else
            {
                ...
                if (arch != nullptr)
                {
                    ...
                    if (readResult.validObject())
                    {
                        ...
                        if (result != nullptr && options != nullptr && (options->getObjectCacheHint() & osgDB::ReaderWriter::Options::CACHE_OBJECTS))
                        {
            ...
            else
            {
                ...
                if (arch != nullptr)
                {
                    ...
                    if (readResult.validNode())
                    {
                        ...
                        if (result != nullptr && options != nullptr && (options->getObjectCacheHint() & osgDB::ReaderWriter::Options::CACHE_NODES))
                        {
                            reg->addEntryToObjectCache(strippedFilename, result);
                        }
                    }
    ...
    FileUtilIOException::FileUtilIOException(const std::string& message, const std::string& filename, unsigned int linenum)
        : trUtil::Exception(message, filename, linenum)
    {
        }
